...
9.  两个有序数列合并
   3. 头数值比较，那个数值合适，就使用哪个，依次向前。

10. 哈希表的实现原理，如何解决冲突
   4. 计算出key的哈希值，然后对哈希值进行数组长度取余（这里长度为素数），得到的为bucket位置，在用冲突解决算法（C#中为拉链法）计算出数据的索引位置。
   5. 可参考上方Dictionary实现原理。
   6. HashTable与Dictionary区别参考：https://blog.csdn.net/mpegfour/article/details/78725768
